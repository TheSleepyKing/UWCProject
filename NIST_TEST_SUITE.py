...
"""
The test in this file are taken from github user stevenang (https://github.com/stevenang/randomness_testsuite)
These will be used to throughly test the random number generators developed, these are not code that I have
developed, but more as a mean to test the code that has been developed for the application of random number 
generators. 

The code below is taken from the implementation from this document
NIST documentation http://csrc.nist.gov/publications/nistpubs/800-22-rev1a/SP800-22rev1a.pdf
...
def random_excursions_test(binary_data:str, verbose=False, state=1):
    ...
    pi = ([([get_pi_value(uu, state) for uu in range(6)]) for state in x_values])
...
def variant_test(binary_data:str, verbose=False):
    ...
    for count in (sorted(set(index))):
        if not count == 0:
            ...
            p_values.append(erfc(abs(get_frequency(li_data, count) - j) / den))
